...
/// Returns the Roaming AppData directory.
///
/// Checks the `APPDATA` environment variable first (for portable mode
/// support where `set_var("APPDATA", ...)` redirects data next to the
/// executable), then falls back to the Win32 Known Folder API.
...
/// Returns the Local AppData directory.
///
/// Checks the `LOCALAPPDATA` environment variable first (for portable mode
/// support where `set_var("LOCALAPPDATA", ...)` redirects data next to the
/// executable), then falls back to the Win32 Known Folder API.
